...
namespace UpdateControls.Correspondence.UnitTest
{
    ...
    public class FieldSerializerTest
    {
        ...
        {
            DecimalFieldSerializer serializer = new DecimalFieldSerializer();

            TestSerializer(0.0m,  new byte[] { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00 }, serializer);
            TestSerializer(1.0m,  new byte[] { 0x0A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00 }, serializer);
            TestSerializer(1.1m,  new byte[] { 0x0B, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00 }, serializer);
            TestSerializer(2.0m,  new byte[] { 0x14, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00 }, serializer);
            TestSerializer(2.1m,  new byte[] { 0x15, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00 }, serializer);
            TestSerializer(-1.0m, new byte[] { 0x0A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x80 }, serializer);
            TestSerializer(-2.0m, new byte[] { 0x14, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x80 }, serializer);

            TestSerializer(10.0m, new byte[] { 0x64, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00 }, serializer);
            TestSerializer(10.1m, new byte[] { 0x65, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00 }, serializer);
            TestSerializer(20.0m, new byte[] { 0xC8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00 }, serializer);
            TestSerializer(20.1m, new byte[] { 0xC9, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00 }, serializer);

            TestSerializer(100.0m, new byte[] { 0xE8, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00 }, serializer);
            TestSerializer(200.0m, new byte[] { 0xD0, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00 }, serializer);

            TestSerializer(0.01m, new byte[] { 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00 }, serializer);
            TestSerializer(0.02m, new byte[] { 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00 }, serializer);
        }
        ...
        {
            BinaryFieldSerializer serializer = new BinaryFieldSerializer();

            // Special case: null equals empty array.
            using (MemoryStream stream = new MemoryStream())
            {
                using (BinaryWriter output = new BinaryWriter(stream))
                {
            ...
            TestSerializer(new byte[0], new byte[] { 0x00, 0x00 }, serializer);
            TestSerializer(new byte[] { 0x01, 0x02, 0x03 }, new byte[] { 0x00, 0x03, 0x01, 0x02, 0x03 }, serializer);
        }
        ...
        private static void TestSerializer(byte[] value, byte[] data, IFieldSerializer serializer)
        {
            using (BinaryReader reader = new BinaryReader(new MemoryStream(data)))
            {
                byte[] read = (byte[])serializer.ReadData(reader);
                Assert.IsTrue(ArraysEqual(value, read));
            }
