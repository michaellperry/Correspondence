...
{
    public class SQLStorageStrategy : IStorageStrategy
    {
        ...
        public SQLStorageStrategy(string connectionString)
		{
        ...
        public async Task<FactID?> GetIDAsync(string factName)
        {
            using (var session = new Session(_connectionString))
            {
                ...
                {
                    return new FactID() { key = (System.Int64)result };
                }
            }
        }
        ...
        public async Task<SaveResult> SaveAsync(FactMemento memento, int peerId)
		{
			...
			using (var session = new Session(_connectionString))
			{
				...
				if (FindExistingFact(memento, out id, session))
                    return new SaveResult { WasSaved = false, Id = id };
                ...
                return new SaveResult { WasSaved = true, Id = id };
			}
		}
        ...
        public async Task<List<MessageMemento>> LoadRecentMessagesForServerAsync(int peerId, TimestampID timestamp)
        {
            using (var session = new Session(_connectionString))
            {
                session.Command.CommandText = "SELECT TOP (20) PivotId, FactId FROM Message WHERE FactId > @Timestamp AND PeerId != @PeerId ORDER BY FactId";
        ...
        public async Task<FactID?> GetRemoteIdAsync(FactID localFactId, int peerId)
        {
            ...
            {
                // See if the share already exists.
                session.Command.CommandText = "SELECT RemoteFactId FROM Share WHERE FKPeerID = @PeerID AND FKFactID = @LocalFactID";
                AddParameter(session.Command, "@PeerID", peerId);
                AddParameter(session.Command, "@LocalFactID", localFactId.key);
                using (IDataReader shareReader = session.Command.ExecuteReader())
                {
                    ...
                    {
                        // It does.
                        return new FactID { key = shareReader.GetInt64(0) };
                    }
                }
                return null;
            }
        ...
        public SQLStorageStrategy UpgradeDatabase()
        {
            using (var session = new Session(_connectionString))
            {
        ...
        {
            return Assembly.GetExecutingAssembly().GetManifestResourceStream(typeof(SQLStorageStrategy), String.Format(@"Scripts.{0}", name));
        }
